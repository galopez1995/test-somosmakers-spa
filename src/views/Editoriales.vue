  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12">
        <v-toolbar flat color="primary" dense>
          <v-toolbar-title class="white--text">Listado de editoriales en el sistema</v-toolbar-title>
        </v-toolbar>
        <v-card>
    ...
    <app-dialog width="50%" v-if="form.dialog">
      <template v-slot:title>
        <span class="headline white--text">Editorial</span>
        <v-spacer></v-spacer>
      ...
      <v-form ref="formRecord" v-model="form.valid" lazy-validation>
        <v-container>
          <v-row>
            <v-col cols="12" md="12">
              <v-text-field v-model="form.record.nombre" label="Nombre (*)" dense outlined clearable :rules="[...form.rules.required]"></v-text-field>
            </v-col>
          </v-row>
  ...
  export default Vue.extend({
    data() {
      return {
        help: {
          title: 'Editoriales',
          subtitle: '',
          message: 'En esta página podrás encontrar todas las editoriales que han sido creados.'
        },
    ...
    methods: {
      async load() {
        ...
        const records = await EditorialService.findAll()
        this.table.records = records as any
        this.$store.dispatch('app/hideLoading')
      },

      add() {
        this.form.record = {} as IEditorialModel
        this.form.dialog = true
      },

      async edit(item: IEditorialModel) {
        this.$store.dispatch('app/showLoading')
        this.form.record = (await EditorialService.findById(item)) as any
        this.form.dialog = true
        this.$store.dispatch('app/hideLoading')
      },
      ...
      async save() {
        if (this.formRecord.validate()) {
          ...
          if (this.form.record.idEditorial > 0) response = await EditorialService.update({ ...this.form.record } as IEditorialModel)
          else response = await EditorialService.save({ ...this.form.record } as IEditorialModel)
          this.$store.dispatch('app/hideLoading')
